  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$dashboard <- renderUI({
      ...
      tagList(
        ...
        div(class = "comp-freshness", "Estimated cutoffs. Final levels will be set using end-of-season ADL salary rankings.", tags$br(),
        paste("Source snapshot:", s$source_at)),
        ...
        tags$details(tags$summary("Qualification thresholds & rules"),
          p(strong("Who counts? "), paste("A compensatory free agent (CFA) is a player whose qualifying June auction winning bid amount meets the minimum shown above. This tracker compares your", s$season - 1L, "roster with qualifying acquisitions in", s$season, "within your conference. Re-signing your own player has no effect.")),
          p(strong("Which dates count? "), paste("Eligible auction wins run from", format(s$ufa_window$start, "%B %d, %Y at %I:%M %p", tz = "America/New_York"), "Eastern until July 1 (exclusive). A trade counts only if it occurs on a later Eastern calendar day than the qualifying auction win. Trades after July 1 can still count.")),
